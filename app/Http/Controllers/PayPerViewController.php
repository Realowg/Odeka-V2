...
class PayPerViewController extends Controller
{
  ...
  protected function sendStripe()
  {
    ...
    $cents  = in_array(config('settings.currency_code'), config('currencies.zero-decimal')) ? Helper::amountGross($this->request->amount) : (Helper::amountGross($this->request->amount) * 100);
    $amount = (int)$cents;
    $currency_code = $this->settings->currency_code;
    $description = __('general.unlock_content') . ' @' . $media->user()->username;
