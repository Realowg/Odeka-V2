...
<html lang="{{ str_replace('_','-', app()->getLocale()) }}">
  <head>
    ...
    <title>{{ __('auth.reset_password') }} — {{ config('settings.title') }}</title>
  ...
  <body class="min-h-screen bg-neutral-950 text-neutral-100">
    ...
    <main class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
      <div class="mx-auto w-full max-w-md rounded-3xl border border-neutral-900 bg-neutral-950/60 backdrop-blur p-6 sm:p-8">
        <h1 class="text-center text-2xl sm:text-3xl font-semibold tracking-tight">{{ __('auth.reset_password') }}</h1>
        <p class="mt-1 text-center text-sm text-neutral-400">{{ __('auth.reset_pass_subtitle') }}</p>
        ...
        <form method="POST" action="{{ url('password/reset') }}" id="passwordResetForm" class="mt-6 space-y-4">
          @csrf
          ...
          <div>
            <label for="password_confirmation" class="block text-sm text-neutral-300">{{ __('auth.confirm_password') }}</label>
            <input id="password_confirmation" name="password_confirmation" type="password" required class="mt-1 w-full rounded-xl border border-neutral-800 bg-neutral-950 px-4 py-3 text-sm outline-none focus:border-neutral-600" />
          </div>
          @if ($settings->captcha == 'on')
            {!! NoCaptcha::displaySubmit('passwordResetForm', __('auth.reset_password'), ['data-size' => 'invisible', 'class' => 'w-full rounded-full bg-white text-neutral-900 px-6 py-3 text-sm font-medium hover:bg-neutral-200']) !!}
            {!! NoCaptcha::renderJs() !!}
          @else
            <button type="submit" class="w-full rounded-full bg-white text-neutral-900 px-6 py-3 text-sm font-medium hover:bg-neutral-200">{{ __('auth.reset_password') }}</button>
          @endif
        </form>
      </div>
    </main>
