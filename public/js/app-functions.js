...
(function ($) {
  ...
  function copyToClipboard(element, btn) {
    ...
    $(btn).html(
      '<i class="fa fa-check"></i> <span class="btn-block mt-3">' +
        copied +
        "</span>",
    );
    setTimeout(function () {
      $(btn).html(
        '<i class="fas fa-link"></i> <span class="btn-block mt-3">' +
          copy_link +
          "</span>",
      );
    }, 1000);
  ...
  function textTruncate(element, text) {
    ...
    if (descHeight > 500) {
      $(element)
        .addClass("truncate")
        .append(
          '<span class="btn-block text-center color-default font-default readmoreBtn"><strong>' +
            text +
            "</strong></span>",
        );
    }
  ...
  function toggleNavbarMethod() {
    if ($(window).width() > 768) {
      ...
      $(".btn-register-menu")
        ...
        .addClass("btn-primary mobileButton");
    }
  }
  toggleNavbarMethod();
  ...
  if ($(document).scrollTop() > $(".scroll").height()) {
    ...
    $(".scroll").addClass(
      "shadow-custom navbar_background_color p-nav-scroll link-scroll",
    );
  ...
  // Safe timeago initialization with fallback
  if (typeof jQuery !== "undefined" && jQuery.fn.timeago) {
    jQuery(".timeAgo").timeago();
  } else {
    console.warn("Timeago library not available");
  }
  ...
  $(document).on("click", "#btnCreateUpdate", function (s) {
    ...
    element
      .find("i")
      .addClass("spinner-border spinner-border-sm align-middle mr-1");
    ...
    if (trim(description).length == 0) {
      ...
      $("#showErrorsUdpate").html(
        '<li><i class="fa fa-times-circle"></i> ' + $empty + "</li>",
      );
          ...
          success: function (result) {
            ...
            if (result.success != false && !result.pending) {
              ...
              if (result.total == 1) {
                ...
                $(result.data)
                  .hide()
                  .insertBefore(".card-updates:first")
                  .fadeIn(500);
              }
  ...
  $(document).on("keypress", ".comments", function (e) {
    if (e.which == 13) {
      var element = $(this);
      var isReplyTo = element
        .parents(".container-comments ")
        .find(".isReplyTo");
      var inputIsReply = element
        .parents(".container-comments ")
        .find(".isReply");
      e.preventDefault();
      element.blur();
      ...
      $.ajax({
        ...
        success: function (result) {
          if (result.success) {
            ...
            element
              .parents(".card-footer")
              .find(".dangerAlertComments")
              .fadeOut(1);

            if (result.isReply) {
              element
                .parents(".card-footer")
                .find(".wrap-comments" + result.idComment)
                .append(result.data);
              // Safe timeago initialization with fallback
              if (typeof jQuery !== "undefined" && jQuery.fn.timeago) {
                jQuery(".timeAgo").timeago();
  ...
  $(".updateDescription").on("keyup", function () {
    ...
    if (trim(element).length >= 1) {
      $(this)
        .parents("form")
        .find(".btnEditUpdate, .btnEditComment")
        .removeAttr("disabled")
        .removeClass("e-none");
      return false;
    } else {
  ...
  clip.on("success", function () {
    $(".popout")
      ...
      .html('<i class="fa fa-check mr-1"></i> ' + copiedSuccess)
      .slideDown("200")
      .delay("3000")
      .slideUp("50");
  });
  ...
  $(document).ready(function () {
    if (Cookies.get("announcementsAlert" + announcement_cookie));
      ...
      $("#closeAnnouncements").on("click", function () {
        ...
        Cookies.set("announcementsAlert" + announcement_cookie, true, {
          expires: 365,
        });
      });
    }
  ...
  $(document).on("click", ".loadMoreComments", function (e) {
    ...
    wrapContainer.html(
      '<a href="javascript:void(0)"><span class="line-replies"></span>' +
        loading +
        "</a>",
    );
    ...
    $.ajax({
      url:
        URL_BASE + "/loadmore/comments?post=" + post + "&skip=" + allElements,
      dataType: "json",
    })
      .done(function (data) {
        if (data.comments) {
  ...
  $(document).on("click", "#updatesPaginator .loadPaginator", function (e) {
    ...
    if (is_profile == true) {
      var url_pagination =
        ...
        "id=" +
        profile_id +
        sort_post_by_type_media +
        "&skip=" +
        allElements;
    } else if (is_bookmarks == true) {
      var url_pagination =
        URL_BASE + "/ajax/user/bookmarks?skip=" + allElements;
    } else if (is_likes == true) {
      var url_pagination = URL_BASE + "/ajax/user/likes?skip=" + allElements;
    } else if (is_purchases == true) {
      var url_pagination =
        URL_BASE + "/ajax/user/purchases?skip=" + allElements;
    } else if (is_explore == true) {
      var url_pagination =
        URL_BASE + "/ajax/explore?" + filter + "" + q + "skip=" + allElements;
    } else {
  ...
  $("#contentLocked").on("click", function () {
    if ($(this).hasClass("unlock")) {
      ...
      $(this)
        .html('<i class="feather icon-lock f-size-20 align-bottom"></i>')
        .removeClass("unlock");
    } else {
      $("#customCheckLocked").trigger("click");
      $(this)
        .html('<i class="feather icon-unlock f-size-20 align-bottom"></i>')
        .addClass("unlock");
    }
  });
  ...
  $(document).on("click", ".contentLocked", function () {
    if ($(this).hasClass("unlock")) {
      ...
      $(this)
        .html('<i class="feather icon-lock" style="font-size:25px;"></i>')
        .removeClass("unlock");
    } else {
      $(this).parents("form").find(".customCheckLocked").trigger("click");

      $(this)
        .html('<i class="feather icon-unlock" style="font-size:25px;"></i>')
        .addClass("unlock");
    }
  });
    ...
    if (window.File && window.FileReader && window.FileList && window.Blob) {
      ...
      if ($(this).val()) {
        var oFReader = new FileReader(),
          rFilter =
            /^(?:image\/gif|image\/ief|image\/jpeg|image\/jpeg|image\/jpeg|image\/png|image|video\/mp4|video\/quicktime|audio\/mpeg)$/i;
        ...
        if (ftype == "audio/mpeg") {
          ...
          $("#previewImage").html(
            '<i class="fa fa-music text-info mr-1"></i> ' + $fileName,
          );
          $("#removePhoto").show();
        }
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data) {
              ...
              $("#containerUsers").html(
                '<small class="text-center">' + no_results_found + "</small>",
              );
              $("#spinner").hide();
            }
          },
  ...
  $("#save").on("click", function (e) {
    ...
    $element
      ...
      .html(
        '<i class="spinner-border spinner-border-sm align-middle mr-1"></i>',
      );
  ...
  $(document).on("click", ".btnFilePhoto", function () {
    var _this = $(this);
    _this
      .parents(".previewImageVerification")
      .find(".fileVerifiyAccount")
      .trigger("click");
    _this.blur();
  });
  ...
  $(".fileVerifiyAccount").on("change", function () {
    ...
    if (window.File && window.FileReader && window.FileList && window.Blob) {
      ...
      if ($(this).val()) {
        var oFReader = new FileReader(),
          rFilter =
            /^(?:image\/gif|image\/ief|image\/jpeg|image\/jpeg|image\/jpeg|image\/png|image)$/i;
        if ($(this)[0].files.length === 0) {
          return;
        }
        ...
        _this
          ...
          .find(".previewImage")
          .html(
            '<i class="fas fa-image text-info mr-1"></i> <strong>' +
              oFile.name +
              "</strong>",
          );
      }
        ...
        if (response.success != false) {
          if (response.status == "pin") {
            ...
            $(".pinned-current")
              .parents(".card-updates")
              .find(".pin-post")
              .html('<i class="bi bi-pin mr-2"></i> ' + pin_to_your_profile);
            $(".pinned-current").hide();
            element.html(
              '<i class="bi bi-pin mr-2"></i> ' + unpin_from_profile,
            );
            element
              .parents(".card-updates")
              .find(".pinned_post")
              .addClass("pinned-current");

            element
              .parents(".card-updates")
              .addClass("pinned-post", function () {
            ...
            $(".popout")
              .removeClass("popout-error")
              .addClass("popout-success")
              .html('<i class="fa fa-check mr-1"></i> ' + post_pinned_success)
              .slideDown("200")
              .delay("3000")
              .slideUp("50");
          } else {
            element
              .parents(".card-updates")
              .find(".pinned_post")
              .removeClass("pinned-current");
            element.html(
              '<i class="bi bi-pin mr-2"></i> ' + pin_to_your_profile,
            );

            element
              .parents(".card-updates")
              .removeClass("pinned-post", function () {
            ...
            $(".popout")
              .removeClass("popout-error")
              .addClass("popout-success")
              .html('<i class="fa fa-check mr-1"></i> ' + post_unpinned_success)
              .slideDown("200")
              .delay("3000")
              .slideUp("50");
          }
  ...
  $(document).on("mouseenter", ".subscriptionActive", function () {
    $(this).html(
      '<i class="feather icon-user-x mr-1"></i> ' + cancel_subscription,
    );
  ...
  $(document).on("mouseleave", ".subscriptionActive", function () {
    $(this).html(
      '<i class="feather icon-user-check mr-1"></i> ' + your_subscribed,
    );
    ...
    $.ajax({
      ...
      success: function (result) {
        if (result.success) {
          ...
          element
            .find("i")
            .removeClass("spinner-border spinner-border-sm align-middle mr-1");
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data) {
              ...
              $("#containerCreators").html(
                '<span class="p-2 text-center w-100 d-block">' +
                  no_results +
                  "</span>",
              );
  ...
  $(document).on("click", ".refresh_creators", function (e) {
    ...
    var btnFreeCreators = element
      .parents(".filter-explorer")
      .find(".toggleFindFreeCreators");
    ...
    $.post(
      ...
      function (data) {
        if (data.success == true) {
          if (data.type == "like") {
            element.html(
              '<i class="fas fa-heart text-red mr-1"></i> <span class="countCommentsLikes">' +
                data.count +
                "</span>",
            );
            element.blur();
            ...
            element.html(
              '<i class="far fa-heart mr-1"></i> <span class="countCommentsLikes">' +
                $count +
                "</span>",
            );
  ...
  $(document).on("click", ".loadMoreReplies", function (e) {
    ...
    $.ajax({
      url:
        URL_BASE + "/replies/loadmore?id=" + comment + "&skip=" + allElements,
      dataType: "json",
    })
      .done(function (data) {
        if (data.replies) {
        ...
        .ajaxForm({
          ...
          success: function (result) {
            ...
            if (result.success) {
              ...
              for ($key in result.errors) {
                error +=
                  '<li><i class="fa fa-times-circle mr-1"></i> ' +
                  result.errors[$key] +
                  "</li>";
              }
  ...
  $(document).on("click", ".storyBackgrounds", function (e) {
    ...
    $(".bg-inside").css({
      background: 'url("' + background + '") center center #6a6a6a',
  ...
  $(document).on("click", ".getViews", function (e) {
    ...
    if (total === 0) {
      $("#containerUsers").html(
        '<small class="text-center">' + no_one_seen_story_yet + "</small>",
      );
      $("#spinner").hide();
      return false;
    }
            ...
            if (data.success) {
              if (data.cover) {
                $(".jumbotron-cover-user").css({
                  background: 'url("' + data.cover + '") center center #505050',
                  backgroundSize: "cover",
                });
              } else {
                $(".jumbotron-cover-user").css({
      ...
      form
        .ajaxForm({
          ...
          error: function (responseText, statusText, xhr, $form) {
            ...
            element
              .find("i")
              .removeClass("spinner-border spinner-border-sm align-middle");
          },
          success: function (response) {
            //===== SUCCESS =====//
            if (response.success) {
  ...
  $("#ePubFile").on("change", function () {
    ...
    if (window.File && window.FileReader && window.FileList && window.Blob) {
      ...
      if ($(this).val()) {
        var oFReader = new FileReader(),
          rFilter = /^(?:application\/epub\+zip)$/i;
        if ($(this)[0].files.length === 0) {
          return;
        }
